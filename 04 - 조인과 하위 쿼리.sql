...
SELECT emp_name, emp_id,  employee.dept_id, dept_name, hire_date, salary
	FROM employee 
    JOIN department ON employee.dept_id = department.dept_id;
    
SELECT e.emp_name, e.emp_id,  e.dept_id, d.dept_name, e.hire_date, e.salary
	FROM employee AS e
...
SELECT * FROM unit;
SELECT e.emp_name, e.gender, v.emp_id, d.dept_name, v.begin_date, v.reason, v.duration
	FROM vacation AS v
...
SELECT e.emp_id, e.emp_name, u.unit_name, e.phone, e.email
	FROM employee AS e
	INNER JOIN department AS d ON e.dept_id = d.dept_id
...
-- 가장 먼접 입사한 사람 정보
-- 밑에건 2013입사자의 정보지 먼저 입사한 사람정보가아님
...
SELECT emp_name, emp_id, dept_id, hire_date, salary
	FROM employee 
    WHERE dept_id in (SELECT dept_id FROM department WHERE unit_id= 'B');
...
-- Q) 강우동(S0003)보다 급여를 많아 받는 직원 정보 조회
