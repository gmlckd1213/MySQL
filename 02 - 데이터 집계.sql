...
SELECT  emp_name, dept_id,gender,SUM(salary) AS salary_sum
FROM employee
WHERE retire_date IS NULL
GROUP BY dept_id;
...
SELECT COUNT(*) AS cnt, SUM(salary), AVG(salary) AS tot_salary, AVG(COALESCE(salary, 0)) AS avg_salary
	FROM employee
    WHERE gender = 'M';
    
    
SELECT dept_id, 
	   SUM(CASE WHEN gender = 'M' THEN salary ELSE 0 END) AS M_salary,
	   SUM(CASE WHEN gender = 'M' THEN salary ELSE 0 END) AS F_salary,
