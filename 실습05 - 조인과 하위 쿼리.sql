...
-- Q04) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 고객이름, 직원이름도 같이 조회되게 수정하세요.
 SELECT o.order_id, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due 
	FROM order_header AS o
    JOIN customer AS c ON c.customer_id = o.customer_id
    JOIN employee AS E ON o.employee_id = e.employee_id
    WHERE total_due>8500000;
...
-- Q05) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 전체금액이 8,500,000 이상인 '서울' 지역 고객만 조회되게 수정하세요.
 SELECT o.order_id, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due, c.city
	FROM order_header AS o
    JOIN customer AS c ON c.customer_id = o.customer_id
    JOIN employee AS E ON o.employee_id = e.employee_id
    WHERE total_due>8500000
...
-- Q06) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 전체금액이 8,500,000 이상인 '서울' 지역 '남자' 고객만 조회되게 수정하세요.
 ...
 SELECT o.order_id, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due, c.city, c.gender
	FROM order_header AS o
    JOIN customer AS c ON c.customer_id = o.customer_id
    JOIN employee AS E ON o.employee_id = e.employee_id
    WHERE total_due>8500000
...
-- Q08) 위에서 작성한 쿼리문을 복사해서 붙여 넣은 후 상품이름도 같이 조회되도록 수정하세요.
  ...
  SELECT o.order_id, p.product_id, o.order_qty, o.unit_price, o.line_total
...
-- Q10) 위에서 작성한 쿼리문을 복사해서 붙여 넣은 후 소분류이름, 대분류아이디가 조회되게 수정하세요.
  ...
  FROM product as p
  INNER JOIN sub_category AS s ON p.sub_category_id = s.sub_category_id;
...
SELECT pr.product_id,  pr.product_name,  pr.sub_category_id,  sc.sub_category_name, sc.category_id, c.category_name
FROM product AS pr
INNER JOIN sub_category AS sc ON pr.sub_category_id = sc.sub_category_id
...
SELECT c.category_name,  sc.sub_category_name,  pr.product_name, pr.product_id
FROM product AS pr
INNER JOIN sub_category AS sc ON pr.sub_category_id = sc.sub_category_id
INNER JOIN category AS c ON sc.category_id = c.category_id;
